...
public class Report_bill_config_function
{
	...
	public Report_bill_config_function()
	{
    ...
    {
        Report_bill_configurationDAO rdobj = new Report_bill_configurationDAO();
        rdobj = (Report_bill_configurationDAO)arraylist[0];
        ...
        try
        {
            ...
            #region SaveData
            reportid = rptc.fetchReportID("Bill Report");
            ...
            if (rdobj.Sz_appointment_Date != null)
            {
                filterCode = rptc.fetchFilterCode("sz_accident_date");
                selectedValues = Convert.ToString(rdobj.Sz_appointment_Date);

                if (rdobj.Sz_appointment_Date != "")
                    wherecls = " AND  MST_PATIENT.DT_INJURY='" + rdobj.Sz_appointment_Date + "' ";
                else
                    wherecls = "";
                arrParam.Add(rdobj.CompanyID);
                arrParam.Add(reportid);
