...
public partial class Bill_Sys_EmailVerfication : PageBase
{
    ...
    {
        string token = Convert.ToString(Request.QueryString["token"]).Trim();
        txtToken.Text = token;
        if (String.IsNullOrEmpty(token) || String.IsNullOrEmpty(ValidateAccountVeriToken(token)))
        {
            lblErrorMsg.Text = "Reset Link Invalid";
        }
        

    }
    public string ValidateAccountVeriToken(string token)
    {
        ...
        try
        {
            ...
            sqlString.Append("SELECT sz_USER_NAME FROM MST_USERS");
            sqlString.Append(" WHERE  sz_token=@token AND dt_token_expiry >= SYSDATETIME()");
    ...
    public string ValidateUserAndToken(string token, string userName, string password)
    {
        string id = string.Format("Id: {0} Uri: {1}", Guid.NewGuid(), HttpContext.Current.Request.Url);
        using (Utils utility = new Utils())
        {
        ...
        try
        {
            ...
            command.Connection = con;
            sqlString.Append("SELECT  isnull(MST_USERS.SZ_USER_ROLE,'') [SZ_USER_ROLE] FROM MST_USERS");
            sqlString.Append(" WHERE  sz_token=@token AND SZ_USER_NAME=@username AND SZ_PASSWORD=@password ");
            command.CommandType = CommandType.Text;
            command.CommandText = sqlString.ToString();
            command.Parameters.AddWithValue("token", token);
    ...
    {
        if (createIdentificationCookie(roles))
        {
    ...
    public bool ActivateUser(string token, string userName, string password)
    {
        string id = string.Format("Id: {0} Uri: {1}", Guid.NewGuid(), HttpContext.Current.Request.Url);
        using (Utils utility = new Utils())
        {
        ...
        try
        {
            command.Connection = con;
            sqlString.Append("UPDATE MST_USERS");
            sqlString.Append(" SET has_contact_info=1, is_active =1, dt_accet_verify_expiry = (DATEADD(day, -1, SYSDATETIME()))");
            sqlString.Append(" WHERE  SZ_USER_NAME=@username");
            sqlString.Append(" AND sz_token=@token");
            sqlString.Append(" AND SZ_PASSWORD=@password ");
            command.CommandType = CommandType.Text;
            command.CommandText = sqlString.ToString();
            command.Parameters.AddWithValue("username", userName);
            command.Parameters.AddWithValue("token", token);
