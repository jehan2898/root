...
//************************** Upload Image***********************************
//--------------------------------------------------------------------------------------
function btnUpload_onclick(sender) {
    debugger;
    setTimeout(function () {
    ...
    var i, strHTTPServer, strActionPage, strImageType,sSourcePage;
    var varImgTypejpeg2 = document.getElementById("imgTypepdf2");
    if (varImgTypejpeg2.checked == true) {
        strImageType = 4;
    }
    //var filename = document.getElementById("txtEnterFileName").value;
    //filename = filename.toLowerCase();
    //if (!filename.includes(".pdf")) {
    //    alert('Entered file name does not have valid extension.');
    //    return;
    //}
    ...
    var QueryString = $('[id$=hdnQueryString]').val();
    
    var CurrentPathName = unescape(location.pathname); // get current PathName in plain ASCII	
    var CurrentPath = CurrentPathName.substring(0, CurrentPathName.lastIndexOf("/") + 1);
    //strActionPage = CurrentPath + "ActionPage.aspx?scanSource=" + scanSource; //the ActionPage's file path , Online Demo:"SaveToDB.aspx" ;Sample: "SaveToFile.aspx";
    sSourcePage = CurrentPath + "Scan.aspx?operation='scan'&" + QueryString;
    strActionPage = CurrentPath + "ScanAction.aspx?operation='scan'&" + QueryString;
   
    //strActionPage = CurrentPath + "Scan.aspx";
    var szFileName = document.getElementById("txtEnterFileName").value;
   var fileExtension = szFileName.substr((szFileName.lastIndexOf('.') + 1));
    fileExtension = fileExtension.toLowerCase();
    if (fileExtension!="pdf") {
       
            szFileName=szFileName+'.pdf'
            document.getElementById("txtEnterFileName").value = szFileName;
        }
            
    var OnHttpUploadSuccess = function (httpResponse) {
        alert('File uploaded successfully');
        //location.reload(true);
        //window.location.href = sSourcePage;
        appendMessage('<b>Upload: </b>');
        checkErrorStringWithErrorCode(0, "Successful.");
        if (strActionPage.indexOf("SaveToFile") != -1) {
            alert("Successful");//if save to file.
        } else {
            //window.location.href = strActionPage;
        }
    };

    var OnHttpUploadFailure = function (errorCode, errorString, httpResponse) {
        alert("Failed to upload file. Error Log - " + httpResponse);
    ...
    if (strImageType == 4) {
        if ((DWObject.SelectedImagesCount == 1) || (DWObject.SelectedImagesCount == DWObject.HowManyImagesInBuffer)) {
            DWObject.HTTPUploadAllThroughPostAsPDF(
                strHTTPServer,
                strActionPage,
                szFileName, OnHttpUploadSuccess, OnHttpUploadFailure
            );
        }
        else {
